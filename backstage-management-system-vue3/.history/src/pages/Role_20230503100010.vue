...
<script setup>
  ...
  function search() {
    let initialLength = data.roleList.length;
    data.roleList.forEach(item => {
      item.name == data.search ? data.roleList.unshift(item) : '';
    })
    let variationalLength = data.roleList.length;
    let differLength = variationalLength - initialLengt
  }
</script>
