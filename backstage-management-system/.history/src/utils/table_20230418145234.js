...
// 添加或者修改table中的数据
export function addOrEditTableList(addOrEditApiFunc, getTableListAgain, formData, row, componentInstance) {
  try {
    // 如果有row就说明是修改数据
    if(row) {
      componentInstance[formData].id = row.id;
      let res = await addOrEditApiFunc(componentInstance[formData])
